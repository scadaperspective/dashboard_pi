...
{     SetDrawSoloInPane(true);
    ...
    m_WindHistUpdTimer.Connect(
        wxEVT_TIMER,
        wxTimerEventHandler(
            DashboardInstrument_WindDirHistory::OnWindHistUpdTimer),
        NULL, this);
#endif // _TACTICSPI_H_
}
    ...
    wxTimerEvent &event)
{
    if (!std::isnan(m_TrueWindDir) && !std::isnan(m_TrueWindSpd)){
        ...
        if (m_SpdRecCnt >= 3 && m_DirRecCnt >= 3) {
            m_IsRunning = true;
            m_SampleCount =
                m_SampleCount < WIND_RECORD_COUNT ?
                                m_SampleCount + 1 : WIND_RECORD_COUNT;
            m_MaxWindDir = 0;
            m_MinWindDir = 360;
            m_MaxWindSpd = 0;
            //data shifting
            for (int idx = 1; idx < WIND_RECORD_COUNT; idx++) {
                if (WIND_RECORD_COUNT - m_SampleCount <= idx)
                    m_MinWindDir =
                        wxMin(m_ArrayWindDirHistory[idx], m_MinWindDir);
                m_MaxWindDir =
                    wxMax(m_ArrayWindDirHistory[idx - 1], m_MaxWindDir);
                m_MaxWindSpd =
                    wxMax(m_ArrayWindSpdHistory[idx - 1], m_MaxWindSpd);
                m_ArrayWindDirHistory[idx - 1] =
                    m_ArrayWindDirHistory[idx];
                m_ArrayWindSpdHistory[idx - 1] =
                    m_ArrayWindSpdHistory[idx];
                m_ExpSmoothArrayWindSpd[idx - 1] =
                    m_ExpSmoothArrayWindSpd[idx];
                m_ExpSmoothArrayWindDir[idx - 1] =
                    m_ExpSmoothArrayWindDir[idx];
                m_ArrayRecTime[idx - 1] =
                    m_ArrayRecTime[idx];
            }
            ...
            if (m_SampleCount < 2) {
                ...
                m_ExpSmoothArrayWindSpd[WIND_RECORD_COUNT - 2] =
                    m_WindSpd;
                m_ArrayWindDirHistory[WIND_RECORD_COUNT - 2] = m_WindDir;
                m_ExpSmoothArrayWindDir[WIND_RECORD_COUNT - 2] =
                    m_WindDir;
            }
            ...
            m_ArrayRecTime[WIND_RECORD_COUNT - 1] =
                wxDateTime::Now().GetTm( );
...
*/
	...
	label1.Printf(wxT("%.0f %s"), m_MaxWindSpdScale,m_WindSpeedUnit.c_str());
    // 3/4 legend
    ...
    if(val1==25 || val1==75)  // it's a .25 or a .75
	  label2.Printf(wxT("%.2f %s"), WindSpdScale, m_WindSpeedUnit.c_str());
	else if (val1 == 50)
	  label2.Printf(wxT("%.1f %s"), WindSpdScale, m_WindSpeedUnit.c_str());
    else
	  label2.Printf(wxT("%.0f %s"), WindSpdScale, m_WindSpeedUnit.c_str());
    // center legend
    WindSpdScale=m_MaxWindSpdScale/2.;
    // center line can either have a .0 or .5 value !
    if((int)(WindSpdScale*10) % 10 == 5)
	  label3.Printf(wxT("%.1f %s"), WindSpdScale, m_WindSpeedUnit.c_str());
    else
	  label3.Printf(wxT("%.0f %s"), WindSpdScale, m_WindSpeedUnit.c_str());
    ...
    val1=(int)((WindSpdScale-(int)WindSpdScale)*100);
    if(val1==25 || val1==75)
 	  label4.Printf(wxT("%.2f %s"), WindSpdScale, m_WindSpeedUnit.c_str());
	else if (val1 == 50)
	  label4.Printf(wxT("%.1f %s"), WindSpdScale, m_WindSpeedUnit.c_str());
	else
	  label4.Printf(wxT("%.0f %s"), WindSpdScale, m_WindSpeedUnit.c_str());
...
void DashboardInstrument_WindDirHistory::DrawForeground(wxGCDC* dc)
{
  ...
  if(!m_IsRunning)
    ...
    while(dir <0 ) dir+=360;
    WindAngle=wxString::Format(wxT("TWD %3.0f"), dir)+DEGREE_SIGN;
  }
  ...
  dc->SetFont(*g_pFontData);
  dc->SetTextForeground(col);
  WindSpeed=wxString::Format(wxT("TWS %3.1f %s "), m_WindSpd, m_WindSpeedUnit.c_str());
